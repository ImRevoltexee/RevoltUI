...
local Release = "RevoltUI Build 1.0"
local RayfieldFolder = "RevoltUI"
local ConfigurationFolder = RayfieldFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'RevoltUI Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"RevoltUI Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"RevoltUI Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('RevoltUI had an issue accessing configuration saving capability.')
		end
	end
end
...
RainbowNeon = {
    ...
    TabBackgroundSelected = Color3.fromRGB(255, 0, 200),
    TabTextColor = Color3.fromRGB(255, 255, 255),
    SelectedTabTextColor = Color3.fromRGB(255, 200, 255),
    ElementBackground = Color3.fromRGB(25, 25, 40),
    ElementBackgroundHover = Color3.fromRGB(35, 0, 70),
    SecondaryElementBackground = Color3.fromRGB(15, 0, 30),
    ElementStroke = Color3.fromRGB(180, 0, 255),
    SecondaryElementStroke = Color3.fromRGB(120, 0, 180),
    SliderBackground = Color3.fromRGB(100, 0, 255),
    SliderProgress = Color3.fromRGB(255, 0, 200),
    SliderStroke = Color3.fromRGB(255, 255, 255),
    ToggleBackground = Color3.fromRGB(30, 0, 50),
    ToggleEnabled = Color3.fromRGB(255, 0, 200),
    ToggleDisabled = Color3.fromRGB(70, 70, 70),
    ToggleEnabledStroke = Color3.fromRGB(255, 0, 200),
    ToggleDisabledStroke = Color3.fromRGB(100, 100, 100),
    ToggleEnabledOuterStroke = Color3.fromRGB(255, 100, 255),
    ToggleDisabledOuterStroke = Color3.fromRGB(65, 65, 65),
    DropdownSelected = Color3.fromRGB(60, 0, 90),
    DropdownUnselected = Color3.fromRGB(30, 0, 60),
    InputBackground = Color3.fromRGB(20, 0, 30),
    InputStroke = Color3.fromRGB(255, 0, 200),
...
local RevoltUI = useStudio and script.Parent:FindFirstChild('RevoltUI') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if RevoltUI:FindFirstChild('Build') and RevoltUI.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('RevoltUI | Build Mismatch')
		print('RevoltUI may encounter issues as you are running an incompatible interface version ('.. ((RevoltUI:FindFirstChild('Build') and RevoltUI.Build.Value) or 'No Build') ..').\n\nThis version of RevoltUI is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, RevoltUI = RevoltUI, useStudio and script.Parent:FindFirstChild('RevoltUI') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

RevoltUI.Enabled = false

if gethui then
	RevoltUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(RevoltUI)
	RevoltUI.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	RevoltUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == RevoltUI.Name and Interface ~= RevoltUI then
			Interface.Enabled = false
			Interface.Name = "RevoltUI-Old"
		end
	end
elseif not useStudio then
...
if RevoltUI.AbsoluteSize.X < minSize.X and RevoltUI.AbsoluteSize.Y < minSize.Y then
...
local Main = RevoltUI.Main
local MPrompt = RevoltUI:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = RevoltUI:FindFirstChild('Drag')
...
RevoltUI.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/RevoltUI/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = RevoltUI.Notifications
...
local function ChangeTheme(Theme)
	...
	RevoltUI.Main.BackgroundColor3 = SelectedTheme.Background
	RevoltUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	RevoltUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	RevoltUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	RevoltUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	RevoltUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	RevoltUI.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		RevoltUI.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		RevoltUI.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(RevoltUI:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("RevoltUI | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("RevoltUI | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('RevoltUI had an issue decoding the configuration file, please try delete the file and reopen RevoltUI.') return end
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("RevoltUI | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--RayfieldLibrary:Notify({Title = "RevoltUI Flags", Content = "RevoltUI was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('RevoltUI | Not seeing your actions in notifications?')
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('RevoltUI Settings', 0, true)

	if TabList['RevoltUI Settings'] then
		TabList['RevoltUI Settings'].LayoutOrder = 1000
	end

	if Elements['RevoltUI Settings'] then
		Elements['RevoltUI Settings'].LayoutOrder = 1000
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if RevoltUI:FindFirstChild('Loading') then
		if getgenv and not getgenv().rayfieldCached then
			RevoltUI.Enabled = true
			RevoltUI.Loading.Visible = true

			task.wait(1.4)
			RevoltUI.Loading.Visible = false
		end
	end
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RayfieldLibrary:Notify({Title = 'Build Mismatch', Content = 'RevoltUI may encounter issues as you are running an incompatible interface version ('.. ((RevoltUI:FindFirstChild('Build') and RevoltUI.Build.Value) or 'No Build') ..').\n\nThis version of RevoltUI is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	Report("window_created", Settings.Name or "Unknown")
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "RevoltUI"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "RevoltUI Interface Suite" then
		LoadingFrame.Version.Text = "RevoltUI UI"
	end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		...
		if isfile and not isfile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"RevoltUI RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("RevoltUI | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with RevoltUI specific development.')
				end
			end
		end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("RevoltUI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with RevoltUI specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			RevoltUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("RevoltUI | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with RevoltUI specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("RevoltUI | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with RevoltUI specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("RevoltUI | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with RevoltUI specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("RevoltUI | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with RevoltUI specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("RevoltUI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with RevoltUI specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
	...
	if not success then warn('RevoltUI had an issue creating settings.') end
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['RevoltUI Settings'])
		end)
	end)
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "RevoltUI Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "RevoltUI Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('RevoltUI Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "RevoltUI Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--	Name = "RevoltUI Example Window",
	--	LoadingTitle = "RevoltUI Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using RevoltUI may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like RevoltUI to get the key from
	...
	----RayfieldLibrary:Notify({Title = "RevoltUI Interface", Content = "Welcome to RevoltUI. These - are the brand new notification design for RevoltUI, with custom sizing and RevoltUI calculated wait times.", Image = 4483362458})
